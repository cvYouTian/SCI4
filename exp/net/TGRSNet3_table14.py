...
from model.block.Gaussian import Gaussian
...
class EdgeBottleNeck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.conv1 = CDC_conv(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = CDC_conv(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)
