...
class PD_FA():
    def __init__(self, nclass, bins, size):
        ...
        self.PD = np.zeros(self.bins + 1)
        self.target = np.zeros(self.bins + 1)
        self.all_pixel = size * size

    def update(self, preds, labels):
        batch_size, _, _, W = preds.shape
        for iBin in range(self.bins + 1):
            # score_thresh = iBin * (255 / self.bins)
            score_thresh = iBin / self.bins
            for batch_idx in range(batch_size):
                predits = np.array((preds[batch_idx] > score_thresh).cpu()).astype('int64')
                if W == 512:
                    predits = np.reshape(predits, (512, 512))
                    labelss = np.array((labels[batch_idx]).cpu()).astype('int64')
                    labelss = np.reshape(labelss, (512, 512))
                elif W == 384:
                    predits = np.reshape(predits, (384, 384))
                    labelss = np.array((labels[batch_idx]).cpu()).astype('int64')
                    labelss = np.reshape(labelss, (384, 384))
                else:
                    predits = np.reshape(predits, (512 // 2, 512 // 2))
                    labelss = np.array((labels[batch_idx]).cpu()).astype('int64')
                    labelss = np.reshape(labelss, (512 // 2, 512 // 2))
    ...
    def get(self, img_num):
        Final_FA = self.FA / ((self.all_pixel) * img_num)
        Final_PD = self.PD / self.target
